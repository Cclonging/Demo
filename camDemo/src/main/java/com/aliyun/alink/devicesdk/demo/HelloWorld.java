...
import com.aliyun.alink.devicesdk.demo.entity.CallBackSample;
...
public class HelloWorld {
    ...
    public static void main(String[] args) {
        ...
        //从文件中读取JSON格式的注入到DeviceInfo中（包含iot平台定义的产品和设备的唯一标识）
        ...
        ALog.d(TAG, deviceInfoData.toString());
     ...
     * @param deviceInfoData
     */
    public void executeScheduler(DeviceInfoData deviceInfoData) throws IllegalAccessException {
        //TODO json文件中读取自定义物模型对应的配置项
...
//        }, 3, 5, TimeUnit.SECONDS);
    ...
    private void testCallBack() throws IllegalAccessException {
        CallBackData data = new CallBackData("CALLBACK_INSTRUCTION", "jdajdlajd", "SUCCESS",
                "", new Date());
        CallBackSample callBackSample = new CallBackSample(pk, dn);
        callBackSample.callBack(data);
    }

    private void testUploadData() throws IllegalAccessException {
        UploadData uploadData = new UploadData(
                "181126BT43TNXMNC", "AUTO_EXECUTE", "894532BYXY28N0X4",
                "5cbea3ca2befa56cc114bbaf", "2019-4-24 9:00:00",
                1, 22, "汉", 0,0,0,"这里放置图片地址"
        );
        DataSample dataSample = new DataSample(pk, dn);
        dataSample.updataData(uploadData);
    }
...
//        sample.shadowDelete();
    ...
    private void testPicUpload(){
        PicSample sample = new PicSample(pk, dn);
        sample.picUpdate(new File("C:\\Users\\Cc\\Desktop\\img.jpg"), pk, dn);
